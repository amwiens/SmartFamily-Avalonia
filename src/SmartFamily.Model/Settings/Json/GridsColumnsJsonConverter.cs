    ...
    internal sealed class GridsColumnsJsonConverter : JsonConverter<IDictionary<string, IReadOnlyList<Tuple<string, int>>>>
    {
        ...
        public override IDictionary<string, IReadOnlyList<Tuple<string, int>>>? Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
        {
            ...
            while (reader.Read())
            {
                ...
                {
                    throw new JsonException("an error occurred while trying to parse the datagrid columns. JsonTokenType was not PropertyName");
                }

                var propertyName = reader.GetString();
                if (string.IsNullOrWhiteSpace(propertyName))
                {
                    throw new JsonException("An error occurred while trying to parse the datagrid columns. Failed to get property name");
                }
        ...
        private static IReadOnlyList<Tuple<string, int>> GetTuples(ref Utf8JsonReader reader, JsonSerializerOptions options)
        {
            switch (reader.TokenType)
            {
                        ...
                        switch (reader.TokenType)
                        {
                            ...
                            case JsonTokenType.Number:
                                if (reader.TryGetInt32(out int index))
                                {
                                    ...
                                    {
                                        throw new JsonException("An error occurred while trying to parse the datagrid columns. Could not get header value.", new ArgumentException("Header should not be null or empty.", nameof(header)));
                                    }
                ...
                default:
                    throw new JsonException($"An error occurred while trying to parse the datagrid columns. '{reader.TokenType}' is not supported");
            }
        ...
        private static void HandleValue(Utf8JsonWriter writer, string key, object objectValue)
        {
            ...
            switch (objectValue)
            {
                ...
                default:
                    throw new JsonException($"Ann error occurred while trying to parse the datagrid columns.", new NotImplementedException(objectValue.GetType().ToString()));
            }
        }
